...
class dma_aes_bus_err_vseq extends dma_base_vseq;
  ...
  task program_and_go(bit gcm, int n_blocks, int chunk_blocks = 0);
  ...
  virtual task body();
    ...
    // Start device responders once; they persist across all iterations. AES uses memory mode (addr
    // incrementing, no handshake), not FIFO mode.
    ...
    for (int i = 0; i < num_iters; i++) begin
      ...
      // Inject bus errors with a moderate per-word probability so most transfers hit at least one
      // erroring beat across the gather/scatter blocks.
      enable_bus_errors(0);
      program_and_go(.gcm(gcm), .n_blocks(4), .chunk_blocks(1));
