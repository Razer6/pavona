...
class dma_aes_error_vseq extends dma_base_vseq;
  ...
  task run_case(string name, bit [1:0] aes_op_raw, bit aes_mode,
    ...
    // so the FSM advances to the legal-combination check (where AES + handshake is rejected).
    if (handshake) begin
  ...
  virtual task body();
    ...
    run_case("with_digest",   2'd1, 1'b0, DmaXfer4BperTxn, 32'd64, 32'd64, 2'd1, 1'b0, 4'd0,
             DmaOpcodeErr);
    // AES + hardware handshake: the LSIO trigger advances the FSM to the legal-combination check,
    // which rejects the combination with DmaOpcodeErr (run_case asserts/releases the trigger).
    run_case("aes_handshake",  2'd1, 1'b0, DmaXfer4BperTxn, 32'd64, 32'd64, 2'd0, 1'b1, 4'd0,
             DmaOpcodeErr);
    ...
    // Invalid ASID: program a source ASID that doesn't map to any port. This is a generic config
    // error (not AES-specific) but exercises the DmaAsidErr path for coverage.
    begin
      uvm_reg_data_t d = 0;
      bit [31:0] st, ec;
      ...
      set_src_addr(SrcAddr); set_dst_addr(DstAddr);
      set_src_config(1'b0, 1'b1); set_dst_config(1'b0, 1'b1);
      // Write an unrecognized ASID (0x0) directly; set_addr_space_id constrains to valid values.
      ral.addr_space_id.src_asid.set(4'h0);
      ral.addr_space_id.dst_asid.set(int'(OtInternalAddr));
